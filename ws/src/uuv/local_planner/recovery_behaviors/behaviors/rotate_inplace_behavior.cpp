...
uuv_sys_core::RecoveryState RotateInPlaceBehavior::runBehavior(
  ...
  uuv_sys_core::RecoveryState m_recovery_result;
  auto result = std::make_shared<uuv_sys_core::action::RecoveryBehaviors::Result>();
  while (rclcpp::ok() &&
         (!got_180 ||
          std::fabs(angles::shortest_angular_distance(current_angle, start_angle)) > tolerance_))
  {
    ...
    if(!goal_handle->is_active()){
      ...
      m_recovery_result = uuv_sys_core::RecoveryState::INTERRUPT_BY_NEW_GOAL;
      break;
    }
    if(goal_handle->is_canceling()){
      ...
      m_recovery_result = uuv_sys_core::RecoveryState::INTERRUPT_BY_CANCEL;
      break;
    ...
    if(got_180){
      ...
      m_recovery_result = uuv_sys_core::RecoveryState::RECOVERY_DONE;
      break;
    }
    ...
    if(best_traj.cost_<0){
      ...
      if( valid_time.seconds() > 5.0){
        ...
        m_recovery_result = uuv_sys_core::RecoveryState::RECOVERY_FAIL;
        break;
      }
      geometry_msgs::msg::Twist cmd_vel;
      cmd_vel.linear.x = 0.0;
