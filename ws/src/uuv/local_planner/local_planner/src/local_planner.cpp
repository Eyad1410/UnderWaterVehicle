...
bool Local_Planner::isInitialHeadingAligned(){
  ...
  RCLCPP_WARN(this->get_logger().get_child(name_), "Heading difference from the prune plan starting at %.2f is %.2f", heading_tracking_distance_, yaw);
...
uuv_sys_core::PlannerState Local_Planner::computeVelocityCommand(std::string traj_gen_name, base_trajectory::Trajectory& best_traj){
  
  if(!got_odom_){
    RCLCPP_ERROR(this->get_logger().get_child(name_), "Odom is not received.");
    return uuv_sys_core::TF_FAIL;
  }
  ...
  if(!perception_3d_ros_->getStackedPerception()->isSensorOK()){
    ...
    return uuv_sys_core::PERCEPTION_MALFUNCTION;
  }
  ...
  if((clock_->now()-last_valid_prune_plan_).seconds()>=prune_plane_timeout_){
    ...
    return uuv_sys_core::PRUNE_PLAN_FAIL;
  }
  ...
  for(auto opinion_it=opinions.begin(); opinion_it!=opinions.end();opinion_it++){
    if((*opinion_it)==perception_3d::PATH_BLOCKED_WAIT){
      ...
      return uuv_sys_core::PATH_BLOCKED_WAIT;
    }
      ...
      return uuv_sys_core::PATH_BLOCKED_REPLANNING;      
    }
  }
  ...
  if(best_traj.cost_<0){
    ...
    return uuv_sys_core::ALL_TRAJECTORIES_FAIL;
  }
  else{
    return uuv_sys_core::TRAJECTORY_FOUND;
  }
