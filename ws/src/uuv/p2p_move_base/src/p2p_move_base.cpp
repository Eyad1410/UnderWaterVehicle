...
void P2PMoveBase::handle_accepted(const std::shared_ptr<rclcpp_action::ServerGoalHandle<uuv_sys_core::action::PToPMoveBase>> goal_handle)
{

  if (is_active(current_handle_)){
    RCLCPP_INFO(this->get_logger(), "An older goal is active, cancelling current one.");
    auto result = std::make_shared<uuv_sys_core::action::PToPMoveBase::Result>();
...
void P2PMoveBase::initial(const std::shared_ptr<local_planner::Local_Planner>& lp){
  ...
  global_planner_client_ptr_ = rclcpp_action::create_client<uuv_sys_core::action::GetPlan>(
      this,
  ...
  recovery_behaviors_client_ptr_ = rclcpp_action::create_client<uuv_sys_core::action::RecoveryBehaviors>(
      this,
      "recovery_behaviors", recovery_behaviors_client_group_);

  //@Create action server
  action_server_p2p_move_base_ = rclcpp_action::create_server<uuv_sys_core::action::PToPMoveBase>(
    this,
    "/p2p_move_base",
...
void P2PMoveBase::executeCb(const std::shared_ptr<rclcpp_action::ServerGoalHandle<uuv_sys_core::action::PToPMoveBase>> goal_handle)
{
  auto result = std::make_shared<uuv_sys_core::action::PToPMoveBase::Result>();
  ...
  while(rclcpp::ok()){
    ...
    auto feedback = std::make_shared<uuv_sys_core::action::PToPMoveBase::Feedback>();
...
bool P2PMoveBase::executeCycle(const std::shared_ptr<rclcpp_action::ServerGoalHandle<uuv_sys_core::action::PToPMoveBase>> goal_handle){
    ...
    if(FSM_->isCurrentDecision("d_initial")){
      ...
      if(LP_->isInitialHeadingAligned()){
        ...
        uuv_sys_core::PlannerState PS = LP_->computeVelocityCommand("differential_drive_rotate_shortest_angle", best_traj);

        if(PS == uuv_sys_core::PlannerState::TRAJECTORY_FOUND){
          FSM_->last_valid_control_ = clock_->now();
          FSM_->setDecision("d_align_heading");  
          publishVelocity(best_traj.xv_, best_traj.yv_, best_traj.thetav_);
          return false;
        }
        else if(PS == uuv_sys_core::PlannerState::PERCEPTION_MALFUNCTION){
        ...
        else if(PS == uuv_sys_core::PlannerState::TF_FAIL){
        ...
        else if(PS == uuv_sys_core::PlannerState::PRUNE_PLAN_FAIL){
        ...
        else if(PS == uuv_sys_core::PlannerState::ALL_TRAJECTORIES_FAIL){
        ...
        else if(PS == uuv_sys_core::PlannerState::PATH_BLOCKED_WAIT || PS == uuv_sys_core::PlannerState::PATH_BLOCKED_REPLANNING){
          FSM_->last_valid_plan_ = clock_->now();
          FSM_->setDecision("d_planning");
          publishZeroVelocity();
          return false;
          ...
          RCLCPP_FATAL(this->get_logger(), "Should not happen here, we did not catch uuv_sys_core::PlannerState");
          publishZeroVelocity();
          return false;
        }
      }
      ...
      if(LP_->isGoalHeadingAligned()){
        ...
        if(PS == uuv_sys_core::PlannerState::TRAJECTORY_FOUND){
          ...
          return false;
                ...
                PS == uuv_sys_core::PlannerState::PATH_BLOCKED_WAIT || 
                PS == uuv_sys_core::PlannerState::PATH_BLOCKED_REPLANNING){
          //At least implement last_valid_control_ timeout to abort here
          //@ this assignment will allow at least one time planning query
          if((clock_->now() - FSM_->last_valid_control_).seconds() > FSM_->controller_patience_){
            RCLCPP_WARN(this->get_logger(), "Controller time out, go to recovery");
      ...
      base_trajectory::Trajectory best_traj;
      uuv_sys_core::PlannerState PS = LP_->computeVelocityCommand("differential_drive_simple", best_traj);
      ...
      else if(PS == uuv_sys_core::PlannerState::PATH_BLOCKED_REPLANNING){
        FSM_->last_valid_plan_ = clock_->now();
        publishZeroVelocity();
        FSM_->setDecision("d_planning"); 
      ...
      else if(PS == uuv_sys_core::PlannerState::PATH_BLOCKED_WAIT){
...
void P2PMoveBase::global_planner_client_goal_response_callback(const rclcpp_action::ClientGoalHandle<uuv_sys_core::action::GetPlan>::SharedPtr & goal_handle)
{
  if (!goal_handle) {
...
void P2PMoveBase::startGlobalPlanning(){
  ...
  auto goal_msg = uuv_sys_core::action::GetPlan::Goal();
  goal_msg.goal = FSM_->current_goal_;

  auto send_goal_options = rclcpp_action::Client<uuv_sys_core::action::GetPlan>::SendGoalOptions();
...
void P2PMoveBase::global_planner_client_result_callback(const rclcpp_action::ClientGoalHandle<uuv_sys_core::action::GetPlan>::WrappedResult & result)
{
  switch (result.code) {
...
void P2PMoveBase::startRecoveryBehaviors(){
  ...
  auto goal_msg = uuv_sys_core::action::RecoveryBehaviors::Goal();
  goal_msg.behavior_name = "rotate_inplace";

  auto send_goal_options = rclcpp_action::Client<uuv_sys_core::action::RecoveryBehaviors>::SendGoalOptions();
...
void P2PMoveBase::recovery_behaviors_client_goal_response_callback(const rclcpp_action::ClientGoalHandle<uuv_sys_core::action::RecoveryBehaviors>::SharedPtr & goal_handle)
{
  if (!goal_handle) {
...
void P2PMoveBase::recovery_behaviors_client_result_callback(const rclcpp_action::ClientGoalHandle<uuv_sys_core::action::RecoveryBehaviors>::WrappedResult & result)
{
