...
void ENUTransform::init(){
  
  //Subscriber and Publisher
  odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>("mavros/global_position/local",  rclcpp::QoS(rclcpp::KeepLast(1)).durability_volatile().best_effort(), std::bind(&ENUTransform::odomCb, this, _1));
...
void ENUTransform::geodetic2Ecef(const double latitude, const double longitude, const double altitude, double* x,
                    double* y, double* z)
{
...
void ENUTransform::ecef2Geodetic(const double x, const double y, const double z, double* latitude,
                    double* longitude, double* altitude)
{
...
void ENUTransform::odomCb(const nav_msgs::msg::Odometry::SharedPtr msg)
{
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<ENUTransform>());
